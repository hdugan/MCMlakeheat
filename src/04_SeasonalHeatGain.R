...
ex1 = exSeas |> summarise(heat_J = sum(heat_J, na.rm = T), Area_2D = first(Area_2D), heat_J_m2 = heat_J/Area_2D, 
...
ex2 = exSeas |> 
  ...
  mutate(temp_zero = tempUse - first(tempUse), 
         tempV_zero = tempV - first(tempV)) |> 
  ggplot() +
  geom_path(aes(x = date_time, y = tempV_zero, color = location_name)) +
  geom_point(aes(x = date_time, y = tempV_zero, fill = location_name), shape = 21, stroke = 0.2, size = 2) +
  scale_color_manual(values = c('#4477c9', '#e3dc10', '#b34f0c', '#4c944a'), name = 'Lake') +
  scale_fill_manual(values = c('#4477c9', '#e3dc10', '#b34f0c', '#4c944a'), name = 'Lake') +
  ylab('Mean volumetric temp increase') +
  theme_bw(base_size = 9) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_markdown(),
        legend.position = 'bottom',
...
ex1 / ex2
ggsave('figures/SI_extendedseason.png', width = 4, height = 5, dpi = 500)
