carbonate_speciation <- function(TEMP=16, DIC=2002, pH=8, output = "mg") {
  ...
  if (output == "mol") {
    return(data.frame(co2_molkg = H2CO3, bicarbonate_molkg = HCO3, carbonate_molkg = CO3, alkalinity_eqkg = ALK)) # mol/kg
  } else if (output == "mg") {
    # To convert moles per kg to milligrams per kg (mg/kg), 
    # multiply the bicarbonate result by 61,017.1, the carbonate result by 60,009.2, and the hydroxide result by 17,007.3. 
    ...
    return(data.frame(co2_mgCkg = H2CO3 * 12010, bicarbonate_mgCkg = HCO3*12010, 
                      carbonate_mgCkg = CO3*12010, alkalinity_uEqkg = ALK * 1e6)) # mg C/kg
  }
}